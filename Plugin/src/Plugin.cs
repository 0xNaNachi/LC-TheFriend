...
namespace TheFriend {
    ...
    public class Plugin : BaseUnityPlugin {
        ...
        public static AssetBundle? thefriendassets;
        ...
        private void Awake() {
            ...
            var bundleName = "thefriendassets";
            thefriendassets = AssetBundle.LoadFromFile(Path.Combine(Path.GetDirectoryName(Info.Location), bundleName));
            if (thefriendassets == null) {
                Logger.LogError($"Failed to load custom assets.");
                return;
            }

            // We load our assets from our asset bundle. Remember to rename them both here and in our Unity project.
            var TheFriend = thefriendassets.LoadAsset<EnemyType>("TheFriendObj");
            var TheFriendTN = thefriendassets.LoadAsset<TerminalNode>("TheFriendTN");
            var TheFriendTK = thefriendassets.LoadAsset<TerminalKeyword>("TheFriendTK");

            // Optionally, we can list which levels we want to add our enemy to, while also specifying the spawn weight for each.
            /*
            var TheFriendLevelRarities = new Dictionary<Levels.LevelTypes, int> {
            ...
            var TheFriendCustomLevelRarities = new Dictionary<string, int> {
                {"EGyptLevel", 50},
            ...
            */
            ...
            NetworkPrefabs.RegisterNetworkPrefab(TheFriend.enemyPrefab);
            ...
            Enemies.RegisterEnemy(TheFriend, BoundConfig.configWeight.Value, Levels.LevelTypes.All, TheFriendTN, TheFriendTK);
            // For using our rarity tables, we can use the following:
            // Enemies.RegisterEnemy(TheFriend, TheFriendLevelRarities, TheFriendCustomLevelRarities, TheFriendTN, TheFriendTK);
